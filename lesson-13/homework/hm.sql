...
INSERT INTO Orders1 (CustomerID, OrderID, DeliveryState, Amount) VALUES
...
len(FIRST_NAME) from Employees as [Name_length]
...
Select id, COALESCE(SSN, passportid,itin) as [first_non-null_value] from person
...
select FIRST_NAME, len(FIRST_NAME) - len(replace(FIRST_NAME, 'a', '')) from Employees;
...
WITH MostExperienced AS (
    SELECT
        ...
        SpacemanID,
        MissionCount,
        'Most' AS ExperienceLevel
    FROM (
        SELECT *,
               ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY MissionCount DESC) AS rn
        FROM Personal
    ) AS Ranked
    WHERE rn = 1
),
...
LeastExperienced AS (
    SELECT
        ...
        SpacemanID,
        MissionCount,
        'Least' AS ExperienceLevel
    FROM (
        SELECT *,
               ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY MissionCount ASC) AS rn
        FROM Personal
    ) AS Ranked
    WHERE rn = 1
)
...
-- Combine both
SELECT * FROM MostExperienced
UNION ALL
SELECT * FROM LeastExperienced
ORDER BY JobDescription, ExperienceLevel;
...
SELECT Birthday, COUNT(*) AS NumberOfStudents
FROM Student
GROUP BY Birthday
HAVING COUNT(*) > 1;
...
FROM 
    PlayerScores
GROUP BY 
    LEAST(PlayerA, PlayerB), GREATEST(PlayerA, PlayerB);
