...
From 
	Employees as e
Join 
	Employees as m on e.MANAGER_ID = e.EMPLOYEE_ID
WHERE 
    e.Salary > m.Salary;
...
SELECT 
    'rtcfvty34redt' AS column_name,
    REGEXP_REPLACE('rtcfvty34redt', '[^0-9]', '') AS NumericPart,
    REGEXP_REPLACE('rtcfvty34redt', '[0-9]', '') AS CharPart;
...
WITH RECURSIVE CharList AS (
    SELECT SUBSTRING('sdgfhsdgfhs@121313131', 1, 1) AS char_val, 1 AS position
    UNION ALL
    SELECT SUBSTRING('sdgfhsdgfhs@121313131', position + 1, 1), position + 1
    FROM CharList
    WHERE position < LENGTH('sdgfhsdgfhs@121313131')
)
SELECT char_val
FROM CharList;
...
SELECT EmployeeID,
       ...
       HireDate,
       CASE 
           WHEN DATEDIFF(CURDATE(), HireDate) / 365 < 1 THEN 'New Hire'
           WHEN DATEDIFF(CURDATE(), HireDate) / 365 BETWEEN 1 AND 5 THEN 'Junior'
           WHEN DATEDIFF(CURDATE(), HireDate) / 365 BETWEEN 5 AND 10 THEN 'Mid-Level'
           WHEN DATEDIFF(CURDATE(), HireDate) / 365 BETWEEN 10 AND 20 THEN 'Senior'
           WHEN DATEDIFF(CURDATE(), HireDate) / 365 > 20 THEN 'Veteran'
...
SELECT Id,
       Vals,
       CAST(REGEXP_SUBSTR(Vals, '^[0-9]+') AS UNSIGNED) AS ExtractedInteger
FROM GetIntegers
WHERE Vals IS NOT NULL
  AND REGEXP_SUBSTR(Vals, '^[0-9]+') IS NOT NULL;

SELECT 
    Id,
    -- Swap the first two letters of the string
    CONCAT(SUBSTRING(Vals, 2, 1), SUBSTRING(Vals, 1, 1), SUBSTRING(Vals, 3, LEN(Vals) - 2)) AS SwappedVals
FROM 
    MultipleVals;
