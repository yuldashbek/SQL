...
Select CONCAT_WS(' ', FirstName,LastName) as [EmployeeFullName],Orders.OrderDate from Customers
inner join
...
full join 
Payments as p on o.OrderID = p.OrderID


select Employee.Name as [Employee Name], Manager.Name as [Manager Name] from Employees as Manager 
right join Employees as Employee 
...
select CONCAT_WS(' ',FirstName,LastName) as [FullName],o.Quantity from Customers as c 
inner join 
...
Select d.DepartmentName, COUNT(e.Name) as [EmployeeCount]  from Employees as e
inner join
Departments as d on e.DepartmentID = d.DepartmentID
group by 
d.DepartmentName
...
Select p.ProductName,s.SaleID from Products as p
left join 
...
Select o.OrderID, o.OrderDate, CONCAT_WS (' ' ,FirstName,LastName) as [FullName] from Orders as o
 join 
...
left join 
Payments as p on o.OrderID=p.OrderID
Where OrderDate < '2020-01-01'  and PaymentID is null
...
Select  as ID, Name, EnrollmentID, CourseName  from Students
...
Select o.OrderID, OrderDate,PaymentID from Orders as o
left join Payments as p on o.OrderID= p.OrderID
where PaymentID is null


Select p.ProductID,ProductName,CategoryName from Products as p
