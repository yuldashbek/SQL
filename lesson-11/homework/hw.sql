...
Select o.OrderID, FirstName,OrderDate from Orders as o
...
JOIN 
    (
        ...
        GROUP BY 
            DepartmentID
    ) AS max_salaries ON d.DepartmentID = max_salaries.DepartmentID
JOIN 
    Employees e ON e.DepartmentID = max_salaries.DepartmentID 
                AND e.Salary = max_salaries.MaxSalary;


Select CONCAT_WS(' ',firstName, LastName) as CustomerName, OrderID, OrderDate from Customers as c
 join
...
JOIN (
    ...
    GROUP BY 
        CustomerID
) AS o ON c.firstname = o.CustomerID;
...
Select CONCAT_WS(' ', FirstName,LastName) as CustomerName,OrderDate as MostRecentOrderDate,OrderID from Customers as c
left join  
Orders as o on c.CustomerID= o.CustomerID 
...
Select CONCAT_WS(' ', FirstName,LastName) as CustomerName, OrderID, TotalAmount from Customers as c
join 
Orders as o on c.CustomerID= o.CustomerID 
...
Select ProductName,SaleDate,SaleAmount from Products as p
join 
...
Select ProductName,SaleDate,StockQuantity  from Products as p
Join Sales as s on p.ProductID = s.ProductID
where YEAR(SaleDate) = 2023 and StockQuantity > 50;


Select Name, DepartmentName,HireDate from Employees as e
join Departments as d on e.DepartmentID= d.DepartmentID
Where DepartmentName='Sales' or HireDate > '2020-12-31'



Select CONCAT_WS (' ', FirstName,LastName ) as CustomerName, OrderID, Address, OrderDate from Customers as c
join
Orders as o on c.CustomerID = o.CustomerID
where Country = 'USA'
...
Select ProductName,Category,SaleAmount from Products as p 
join Sales as s on p.ProductID = s.ProductID
Where Category = 'Electronics' and SaleAmount > 350
...
LEFT JOIN (
    ...
    FROM 
        Products
) p ON c.CategoryName = p.Category
GROUP BY 
    c.CategoryName;



Select CONCAT_WS(' ', FirstName,LastName) as CustomerName,City,OrderID,TotalAmount  from Customers as c
join Orders as o on c.CustomerID = o.CustomerID
where City= 'Los Angleles' and TotalAmount> 300



Select Name,DepartmentName from Employees as e
 join Departments as d on e.DepartmentID= d.DepartmentID
 where DepartmentName in ('HR' , 'Finance') or  LENGTH(REGEXP_REPLACE(LOWER(EmployeeName), '[^aeiou]', '', 'g')) >= 4;
